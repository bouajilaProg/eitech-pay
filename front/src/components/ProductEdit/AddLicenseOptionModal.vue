...
const emit = defineEmits(["close", "option-created"]);

const loading = ref(false);

const form = reactive({
  optionId: "",
  licenceId: route.params.product_id,
  optionName: "",
  description: "",
  price: null,
});
...
async function handleSubmit() {
  ...
  try {
    ...
    await licenceService.createOption(payload);

    emit("option-created"); // Tell parent to reload options
    close();
  } catch (err) {
